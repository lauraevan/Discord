...
await p.locator('.modal .dd-control').first().click()
check('the form offers Somewhere Else', (await p.locator('.dd-popout [role="option"]:has-text("Somewhere Else")').count()) === 1)
...
await p.locator('.audit-filter .dd-control').nth(1).click()
const actions = await p.locator('.dd-popout [role="option"]').allInnerTexts()
...
await p.locator('.audit-filter .dd-control').nth(1).click()
await p.locator('.dd-popout [role="option"]').nth(1).click()
await p.waitForTimeout(300)
