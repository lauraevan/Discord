...
export function Composer({
  ...
  const take = (f: File) => {
    if (f.size > uploadCap * 1024 * 1024) {
      ...
      setTooBig(
        `Your files are too powerful. Max upload size is ${uploadCap}MB.`,
      );
      return;
    }
    setTooBig(null);
  ...
  const acts = [
    { label: "Gift a Nitro subscription", Icon: GiftIcon, on: onGiftNitro },
    {
      label: "GIF",
      Icon: GifIcon,
      on: (e: React.MouseEvent) => openPicker(e, "gif"),
    },
    {
      label: "Sticker",
      Icon: StickerIcon,
      on: (e: React.MouseEvent) => openPicker(e, "sticker"),
    },
    {
      label: "Emoji",
      Icon: SmileyIcon,
      on: (e: React.MouseEvent) => openPicker(e, "emoji"),
    },
  ...
  return (
    <div className="composer-wrap">
      ...
      <div className={"composer" + (replyTo ? " replying" : "")}>
        {/* Discord stacks staged uploads on top of the input, inside the same
            rounded box, with a rule between them */}
        {pending.length ? (
          <div className="upload-tray">
            {pending.map((a) => (
              <figure
                key={a.id}
                className={"upload-card" + (a.spoiler ? " spoiler" : "")}
              >
                <div className="upload-thumb">
                  ...
                  {a.spoiler ? (
                    <span className="upload-spoiler-tag">SPOILER</span>
                  ) : null}
                </div>
                <figcaption>{a.name}</figcaption>
                <div className="upload-acts">
                  <Tooltip
                    label={a.spoiler ? "Remove Spoiler" : "Mark as Spoiler"}
                    side="above"
                  >
                    <button
                      ...
                      onClick={() =>
                        setPending((all) =>
                          all.map((x) =>
                            x.id === a.id ? { ...x, spoiler: !x.spoiler } : x,
                          ),
                        )
                      }
                    >
                  ...
                  <Tooltip label="Remove Attachment" side="above">
                    <button
                      ...
                      onClick={() =>
                        setPending((all) => all.filter((x) => x.id !== a.id))
                      }
                    >
                      <TrashIcon />
                    </button>
        ...
        {value.length > limit - 200 ? (
          <span
            className={"composer-count" + (value.length > limit ? " over" : "")}
          >
        ...
        <div className="composer-acts">
          {acts.map(({ label, Icon, on }) => (
            <Tooltip key={label} label={label} side="above">
              <button
                aria-label={label}
                className={label.toLowerCase().replace(/\W+/g, "-")}
                onClick={on}
              >
                <Icon />
              </button>
